...
@Record
private data class SafePlain(val n: Int, val label: String?) : expo.modules.v2.records.Record

/** [SafePlain]'s field types exactly, with both fields declared optional. */
@Record
private data class OptionalFields(val n: Int = 1, val label: String? = "x") : expo.modules.v2.records.Record

@Record(bufferSafe = false)
private class HandleHolder(val target: JavaScriptObject) : expo.modules.v2.records.Record

@Record(bufferSafe = false)
private class DynamicHolder(val meta: Map<String, Any>) : expo.modules.v2.records.Record

@Record(bufferSafe = false)
private class HolderWrapper(val name: String, val inner: List<HandleHolder>) : expo.modules.v2.records.Record

// A handle-free cycle: both members must stay buffer-safe.
@Record
private data class LoopA(val n: Int, val b: LoopB?) : expo.modules.v2.records.Record

@Record
private data class LoopB(val n: Int, val a: LoopA?) : expo.modules.v2.records.Record

// A cycle where one member holds a handle: the taint must reach every cycle member.
@Record(bufferSafe = false)
private class TaintedX(val y: TaintedY?) : expo.modules.v2.records.Record

@Record(bufferSafe = false)
private class TaintedY(val x: TaintedX?, val handle: JavaScriptValue) : expo.modules.v2.records.Record
