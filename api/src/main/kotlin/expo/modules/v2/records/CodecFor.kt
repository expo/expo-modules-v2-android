 ...
 *   `@expo.modules.v2.annotations.Record`, or the module that declares it was built without the
...
fun <T : Record> codecFor(clazz: Class<T>): RecordCodec<T> {
  val type = RecordRegistry.typeFor(clazz)
    ?: throw IllegalArgumentException(
      "${clazz.name} has no RecordCodec - annotate it with @expo.modules.v2.annotations.Record",
    )
