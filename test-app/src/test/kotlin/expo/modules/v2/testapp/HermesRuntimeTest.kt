package expo.modules.v2.testapp
...
import expo.modules.v2.testsupport.TestSupport
import expo.modules.v2.types.CppType
import expo.modules.v2.types.AnyType
import expo.modules.v2.types.buffered
 ...
 */
@Record
private data class Point(val x: Double, val y: Double, val label: String?) : expo.modules.v2.records.Record
...
@Record
private data class MyRecord(val x: Int, val b: String) : expo.modules.v2.records.Record

/** An optional field, so an inbound payload gates `note` with a presence byte. */
@Record
private data class Tagged(val v: Int, val note: String? = null) : expo.modules.v2.records.Record
...
@Record
private data class Pulled(val n: Int, val tag: String, val note: String?) : expo.modules.v2.records.Record
 ...
 */
@Record
private data class Sticker(val id: Int, val label: String) : expo.modules.v2.records.Record
...
@Record
private data class Rogue(val n: Int) : expo.modules.v2.records.Record
 ...
 * plugin will generate (see [expo.modules.v2.args.Trampoline] for the contract). Each JS
...
@Record
private data class Bookmark(val url: URL, val ttl: Duration?) : expo.modules.v2.records.Record
...
class HermesRuntimeTest {
  ...
  fun `a schema pulled from native drives the round trip`() {
    ...
    assertEquals("Lexpo/modules/v2/testapp/Pulled;", data.jniDescriptor)
