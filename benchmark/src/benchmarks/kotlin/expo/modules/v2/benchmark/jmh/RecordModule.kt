 ...
 */
@Record
data class Item(val id: Int, val name: String, val x: Double, val flag: Boolean) : expo.modules.v2.records.Record

@Record
data class Order(val item: Item, val tags: List<String>) : expo.modules.v2.records.Record

/** A record whose schema contains a dynamic field: not buffer-safe, crosses as a Map slot. */
@Record(bufferSafe = false)
data class FlaggedItem(val id: Int, val name: String, val meta: Map<String, Any>) : expo.modules.v2.records.Record
